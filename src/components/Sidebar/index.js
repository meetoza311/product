...
const Sidebar = (props) => {
    const [value, setValue] = useState([70, 300]);
    ...
    // console.log(location.pathname?.includes(`/cat/${item.path_name.toLowerCase()}`))
    return (
        <>
            <div className={`sidebar ${context.isOpenFilters === true && 'open'}`}>
                            ...
                            props?.data.length !== 0 && props?.data.map((item, index) => {
                                return (
                                    <Link to={`/cat/${item.path_name.toLowerCase()}`}>
                                        <div className='catItem d-flex align-items-center' style={{ background: location.pathname?.includes(`/cat/${item.path_name?.toLowerCase()}`) ? 'silver' : '' }}>
                                            <span className='img'><img src={item.items[2].Image} width={30} /></span>
                ...
                <div className='card border-0 shadow'>
                    ...
                    <RangeSlider value={value} onInput={setValue} min={70} max={300} step={5} />
